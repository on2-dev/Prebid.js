...
const BIDDER_CODE = 'alright';
const ENDPOINT_URL = 'https://prebid.alright.network';
const LOG_HEADER = 'Alright Bidder Log: ';
export const spec = {
  code: BIDDER_CODE,
  supportedMediaTypes: [],
  aliases: ['alright'],
  ...
  isBidRequestValid: function(bid) {
    ...
    if (bid.params) {
      if (!bid.params.placementId) {
        utils.logError(LOG_HEADER + 'Param placementId was not defined for bidID ' + bid.bidId);
        isValid = false;
      }
      if (typeof bid.params.cpmFactor != 'undefined' && !bid.params.test) {
        utils.logError(LOG_HEADER + 'Cannot manipulate cpmFactor outside test environment - bidID ' + bid.bidId);
        isValid = false;
      }
      if (bid.params.cpmFactor && (isNaN(bid.params.cpmFactor) || parseInt(Number(bid.params.cpmFactor)) != bid.params.cpmFactor || isNaN(parseInt(bid.params.cpmFactor, 10)))) {
        utils.logError(LOG_HEADER + 'Invalid param definition for cpmFactor on bidID ' + bid.bidId);
        isValid = false;
      }
    } else {
      isValid = false;
      utils.logError(LOG_HEADER + 'No params defined for bidID ' + bid.bidId);
    }
    if (getSizes(bid).length == 0) {
      utils.logError(LOG_HEADER + 'No sizing definition found for bidID ' + bid.bidId);
      isValid = false;
    }
    return isValid;
  },
...
function getPayloadFor(bidRequests, bidderRequest) {
  ...
  if (payload.trackingAllowed) {
    ...
    } catch (error) { utils.logError(LOG_HEADER + 'Location acquisition error - ' + error.toString()); }
  }
  payload.requests = [];
